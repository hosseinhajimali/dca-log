      ...
      <div className="flex items-center gap-3">
        ...
        {isConfirming ? (
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={() => {
                deleteAsset.mutate(asset.id, { onSuccess: () => toast(`${asset.symbol} removed`) });
                onCancelRemove();
              }}
...
export default function Settings() {
  ...
  return (
    <div className="space-y-8 max-w-2xl">
      {showAddModal && (
        <AssetModal mode="add" onClose={() => setShowAddModal(false)} />
      )}
      {editingAsset && (
        <AssetModal mode="edit" asset={editingAsset} onClose={() => setEditingAsset(null)} />
      )}

      <div>
        <h1 className="text-2xl font-bold text-gray-100">Settings</h1>
        <p className="text-sm text-gray-500 mt-1">Manage your preferences and assets</p>
      </div>
        ...
        <div className="px-5 py-4 border-b border-gray-800">
          ...
          <div className="flex flex-wrap gap-2">
            {POPULAR_ASSETS.map(a => (
              ...
              >{assetSymbols.has(a.symbol) ? '✓ ' : '+ '}{a.symbol}</button>
            ))}
          </div>
        </div>
