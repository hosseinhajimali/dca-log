    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60"
      ...
      <div className="w-full max-w-md bg-gray-900 border border-gray-700 rounded-2xl shadow-2xl">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-800">
          ...
          <button onClick={onClose} title="Close" className="text-gray-500 hover:text-gray-200 text-xl leading-none transition-colors">×</button>
        </div>
...
function DeleteAssetModal({ asset, onClose }: { asset: Asset; onClose: () => void }) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60"
      ...
      <div className="w-full max-w-sm bg-gray-900 border border-gray-700 rounded-2xl p-6 space-y-4">
        <div className="flex flex-col gap-2">
          <h2 className="text-base font-semibold text-gray-100">Remove {asset.symbol}?</h2>
          <p className="text-sm text-gray-400">
            This removes <span className="font-mono font-bold text-gray-200">{asset.symbol}</span> ({asset.name}) from your tracked assets. This cannot be undone.
          </p>
        </div>
        <div className="flex gap-3">
          <button
            onClick={() => { deleteAsset.mutate(asset.id, { onSuccess: () => toast(`${asset.symbol} removed`) }); onClose(); }}
...
// ─── asset row ────────────────────────────────────────────────────────────────
function AssetRow({ asset, onEdit, onRemove }: {
  asset: Asset; onEdit: () => void; onRemove: () => void;
}) {
  return (
    <div className="px-5 py-3.5 flex items-center justify-between">
      ...
      <div className="flex items-center gap-2">
        <button onClick={onEdit} title={`Edit ${asset.symbol}`}
        ...
        <button onClick={onRemove} title={`Delete ${asset.symbol}`}
...
export default function Assets() {
  ...
  return (
    <div className="space-y-8">
      ...
      {removingAsset && <DeleteAssetModal asset={removingAsset} onClose={() => setRemovingAsset(null)} />}
      ...
      <section className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
        <div className="px-5 py-4 border-b border-gray-800 flex items-center justify-between">
          ...
          <button onClick={() => setShowAddModal(true)}
            className="text-xs text-brand-400 hover:text-brand-300 border border-brand-500/30 hover:border-brand-500/60 px-3 py-1.5 rounded-lg transition-colors whitespace-nowrap">
            + Add Asset
          </button>
        </div>
